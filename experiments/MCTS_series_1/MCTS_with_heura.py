...
def run_experiment():
    ...
    if not CLUSTER:
        trainer.run_training_games_multi_process(GreedyAgentBoost(), epochs=1, mcts_passes=150, n_test_games=10, exploration_ceofficient=0.61,
    ...
    if CLUSTER:
        trainer.run_training_games_multi_process(GreedyAgentBoost(), epochs=200, mcts_passes=150, n_test_games=96,  exploration_ceofficient=0.61,
                                   experiment_name='MCTS with NN', value_threshold=0.8,
                                                 ...
                                                 confidence_threshold=0.1, count_threshold=6)
