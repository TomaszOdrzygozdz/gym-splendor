...
class Deck:
    ...
    def pop_card_by_id(self,
                        id: int):
            for row in Row:
                ids = [i for i,x in enumerate([x.to_dict() for x in self.decks_dict[row]]) if x == id]
                if ids:
    ...
    def pop_noble_by_id(self,
                        id: int):
        ids = [i for i,x in enumerate([x.to_dict() for x in self.deck_of_nobles]) if x == id]
        if ids:
            return self.deck_of_nobles.pop(ids[0])

    def to_dict(self):
        return [{str(row) : [x.to_dict() for x in self.decks_dict[row]]} for row in Row]
