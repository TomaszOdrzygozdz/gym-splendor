...
class MinMaxAgent(Agent):
    ...
    def choose_action(self, observation) -> Action:
        ...
        if len(self.env.action_space.list_of_actions)>0:
            ...
            self.env.vectorize_state("minmax_state_" + str(numerator) + ".json")
            ...
            self.env.reset()
            self.env.setup_state("minmax_state.json", ordered_deck = True)
    ...
    def restore_env(self, numerator):
        ...
        self.env.setup_state("minmax_state_" + str(numerator) + ".json")
        self.env.update_actions()
