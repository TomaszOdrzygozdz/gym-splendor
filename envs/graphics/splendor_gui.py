...
class SplendorGUI():
    ...
    def draw_card(self,
        ...
        self.drawn_objects.add(
            self.main_canvas.create_text(x_coord + 15, y_coord + 15, fill=CARD_VICTORY_POINTS_FONT_COLOR,
        ...
        for color_index, color in enumerate(card.price.non_empty_stacks()):
            ...
            self.drawn_objects.add(self.drawn_objects.add(self.main_canvas.create_oval(x_coord + 5, y_coord + 40 +
                                                                                       20 * position_index,
                                                                                       x_coord + 20,
                                                                                       y_coord + 55 + 20 * position_index,
                                                                                       fill=color_dict_tkiter[color])))
            self.drawn_objects.add(
                self.main_canvas.create_text(x_coord + 28, y_coord + 48 + 20 * position_index, font=CARD_PRICE_FONT,
    ...
    def draw_noble(self,
        ...
        self.drawn_objects.add(self.main_canvas.create_rectangle(x_coord, y_coord, x_coord + 70, y_coord + 75))
        for position_index, color in enumerate(noble.price.non_empty_stacks()):
            position_x = x_coord + 10
            position_y = 5 + y_coord + 25 * position_index
            self.drawn_objects.add(self.main_canvas.create_rectangle(position_x, position_y, position_x + 15,
                                                                     position_y + 15, fill=color_dict_tkiter[color]))
            self.main_canvas.create_text(position_x + 25, 5 + position_y, font=NOBLE_PRICE_FONT,
                                         fill=NOBLE_BACKGROUND, text=str(noble.price[color]))
    ...
    def draw_board(self,
        ...
        for card in board.cards_on_table:
            position_x = HORIZONTAL_CARD_DISTANCE * len(cards_already_drawn[card.row])
            cards_already_drawn[card.row].append(card)
        ...
        for position_index, noble_card in enumerate(board.nobles_on_board):
            position_x = 20 + x_coord + HORIZONTAL_NOBLE_DISTANCE * position_index
            position_y = y_coord + 550
