...
from envs.mechanics.action import Action, ActionTradeGems, ActionBuyCard, ActionReserveCard
...
def generate_all_legal_buys(state: State) -> List[ActionBuyCard]:
    ...
    for card in all_cards_can_afford:
        ...
        for n_gold_gems_to_use in range(state.active_players_hand().gems_possessed.gems_dict[GemColor.GOLD]+1):
            # we choose combination of other gems:
            options_of_use_gold_as = combinations_with_replacement(card_price_after_discount.non_empty_stacks_except_gold(),
                                                                n_gold_gems_to_use)
