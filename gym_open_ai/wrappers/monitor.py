import gym_open_ai
from gym_open_ai import Wrapper
from gym_open_ai import error, version, logger
import os, json, numpy as np, six
from gym_open_ai.wrappers.monitoring import stats_recorder, video_recorder
from gym_open_ai.utils import atomic_write, closer
from gym_open_ai.utils.json_utils import json_encode_np
...
class Monitor(Wrapper):
    ...
    def _start(self, directory, video_callable=None, force=False, resume=False,
        ...
        if self.env.spec is None:
            logger.warn("Trying to monitor an environment which has no 'spec' set. This usually means you did not create it via 'gym_open_ai.make', and is recommended only for advanced users.")
            env_id = '(unknown)'
        else:
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(directory, ', '.join(training_manifests[:5])))
    ...
    def close(self):
        ...
        logger.info('''Finished writing results. You can upload them to the scoreboard via gym_open_ai.upload(%r)''', self.directory)
...
# not intended for use outside of the gym_open_ai codebase.
def _open_monitors():
...
def collapse_env_infos(env_infos, training_dir):
    ...
    for key in ['env_id', 'gym_version']:
        if key not in first:
            raise error.Error("env_info {} from training directory {} is missing expected key {}. This is unexpected and likely indicates a bug in gym_open_ai.".format(first, training_dir, key))
    return first
